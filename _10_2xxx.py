...
def parse(infile):
    ...
    if (
        x > 0
        and m[x][y] in set(["|", "J", "L", "S"])
        and m[x - 1][y] in set(["F", "7", "|", "S"])
    ):
        # try going up
        #  print("Up")
    ...
    if (
        x < len(m) - 1
        and m[x][y] in set(["|", "F", "7", "S"])
        and m[x + 1][y] in set(["J", "L", "|", "S"])
    ):
        # try going down
        #  print("Down")
        solved, lout = walk(m, l + [[x + 1, y]])
    ...
    if (
        y > 0
        and m[x][y] in set(["7", "J", "-", "S"])
        and m[x][y - 1] in set(["F", "L", "-", "S"])
    ):
        # try going left
        #  print("Left")
    ...
    if (
        y < len(m[0]) - 1
        and m[x][y] in set(["L", "F", "-", "S"])
        and m[x][y + 1] in set(["J", "7", "-", "S"])
    ):
        # try going right
        print("Right")
        solved, lout = walk(m, l + [[x, y + 1]])
...
def sol1(IN):
    ...
    ninside = calculate_inside(l_loop, m)
...
def main():
    with open("Data/_10_1.txt") as infile:
        l = parse(infile)
        #  print(l)
        sol = sol1(l)
        print(sol[0])
